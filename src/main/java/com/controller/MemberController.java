...
<dependency>
	...
	@RequestMapping(value="/idSearch2",method = RequestMethod.GET)
	public String idSearch2(MemberDTO dto,Model m) {
		String n= memberService.idSearch(dto);
		String page = null;
		if(n==null) {
			page="idSearch";
			 m.addAttribute("mesg","값이 바르지 않습니다.");
		}else {
			m.addAttribute("mesg","아이디는"+n+"입니다.");
			page="loginForm";
		}
		return page;
	}
	...
	public String upDate(MemberDTO dto,Model m) {
		System.out.println(dto);
		int n= memberService.memberUpdate(dto);
		if(n==1) {
			m.addAttribute("mesg","회원정보수정완료");
		}else {
			m.addAttribute("mesg","회원정보수정오류");
		}
		return "forward:/";
	}
	...
	public String passSearch2(MemberDTO dto,Model m) {
		String n= memberService.passwdSearch(dto);
		String page = null;
		if(n==null) {
			page="passSearch";
			 m.addAttribute("mesg","값이 바르지 않습니다.");
		}else {
			m.addAttribute("mesg","비밀번호는 "+n+" 입니다.");
			page="loginForm";
		}
		return page;
	}
	...
	public String memberDelete() {
		...
		return "memberDelete";   //main.jsp
	}
	
	@RequestMapping("/memberDelete2")
	public String memberDelete2(HttpSession session) {
		MemberDTO dto=(MemberDTO)session.getAttribute("login");
		String userid=dto.getUserid();
		System.out.println(userid);
		memberService.memberDelete(userid);
		session.invalidate();
		return "deleteDone";   //main.jsp
	}
}
